...
class MariaDBDriver extends EventEmitter {
    static instance;
    _mysql;
    conn;
    config;
    ...
    static createSingleton(config) {
        if (!this.instance) this.instance = new MariaDBDriver(config);
    ...
    checkConnection() {
        if (!this.conn) {
            this.emit("error", {
                message: "MariaDB not connected to the database",
                error: new Error("MariaDB not connected to the database"),
            });
            throw new Error("MariaDB not connected to the database");
        }
    }
    ...
    async connect() {
        try {
            ...
            this.emit("connected", { message: "Successfully connected to the MariaDB database" });
        } catch (error) {
            this.emit("error", { message: "Failed to connect to the MariaDB database", error });
            throw error;
        }
    }
    ...
    async disconnect() {
        try {
            await this.conn.end();
            this.emit("disconnected", { message: "Disconnected from the MariaDB database" });
        } catch (error) {
            this.emit("error", { message: "Failed to disconnect from the MariaDB database", error });
            throw error;
        }
    }
